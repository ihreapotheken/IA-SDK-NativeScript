...
export class IaSdkAndroid extends IaSdkBase {
  ...
  showPharmacyId(completionHandler: (e: any) => void): void {
    Application.android.registerBroadcastReceiver('SHOW_PHARMACY_ID_EVENT', (context, intent) => {
      const message = intent.getStringExtra('data');
      Application.android.unregisterBroadcastReceiver('SHOW_PHARMACY_ID_EVENT');
      setTimeout(() => {
        completionHandler(message);
      }, 0);
    });
    this.iaSdk.showPharmacyId(Utils.android.getCurrentActivity());
  }
